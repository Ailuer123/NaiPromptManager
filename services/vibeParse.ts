import type { VibeEncoding, VibeMember, VibePreset } from '../types';

const VIBE_IDENTIFIER = 'novelai-vibe-transfer';
const VIBE_EXTENSIONS = /(?:\.naiv4vibe(?:bundle)?)?(?:\.json)?$/i;
...
const toMember = (preset: VibePreset): VibeMember => ({
  id: preset.id,
  name: preset.name,
...
const makeGroupPreset = (
  ...
  const groupId = typeof parsed.id === 'string' && parsed.id.length > 0
    ? parsed.id
  ...
  return {
    id: groupId,
    name: displayName,
    thumbnailUrl: members.find(member => member.thumbnailUrl)?.thumbnailUrl,
    encodings: first.encodings,
    members,
    defaultStrength: first.defaultStrength,
    defaultInformationExtracted: first.defaultInformationExtracted,
    sourceFilename,
...
export const parseVibeFileContent = (content: string, filename: string): VibePreset[] => {
  ...
  if (isRecord(parsed) && Array.isArray(parsed.vibes)) {
    ...
    if (vibes.length > MAX_VIBE_IMPORT_COUNT) {
      throw new Error(`单个 bundle 最多包含 ${MAX_VIBE_IMPORT_COUNT} 个 Vibe`);
    }
    const items = vibes.map((item, index) => (
      parseOneVibe(item, filename, vibes.length === 1 ? baseName : `${baseName} ${index + 1}`)
    ));
    if (items.length === 1) return items;
    return [makeGroupPreset(parsed, items.map(toMember), filename, baseName)];
  }
