            ...
            {filteredChains.map((chain) => (
              <div key={chain.id} onClick={() => onSelect(chain.id)} className="group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-indigo-500 dark:hover:border-indigo-500/50 rounded-xl overflow-hidden transition-all duration-200 hover:shadow-xl hover:shadow-indigo-500/10 flex flex-col cursor-pointer">
                ...
                <div className="aspect-[3/4] bg-gray-100 dark:bg-gray-900 relative border-b border-gray-200 dark:border-gray-700 overflow-hidden">
                    {chain.previewImage ? (
                        <img src={chain.previewImage} alt={chain.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                        <div className="flex items-center justify-center h-full text-gray-400 dark:text-gray-700">
                ...
                <div className="p-4 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 truncate pr-2" title={chain.name}>{chain.name}</h3>
                  </div>
                  <p className="text-gray-500 dark:text-gray-400 text-xs mb-4 line-clamp-2 h-8">{chain.description || '暂无描述'}</p>
                  
                  <div className="mt-auto flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-700/50">
                     <span className="text-xs text-gray-400 dark:text-gray-500">{new Date(chain.updatedAt).toLocaleDateString()}</span>
                     <button
